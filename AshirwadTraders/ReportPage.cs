...
namespace AshirwadTraders
{
    public partial class ReportPage : Form
    {
        ...
        private void ComboBoxAccountId_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (comboBoxAccountId.SelectedValue == null)
            {
                return;
            }
            else if (comboBoxAccountId.SelectedValue.Equals("MATERIALS"))
            //else if (comboBoxAccountId.SelectedIndex == 1)
            {
                queryString = "SELECT " +
                "`mtrl_date` as `Date`, 'Self' as `Account`, `mtrl_item` as `Particulars`, `mtrl_rate` as `Rate`, `mtrl_qty` as `Qty`, `mtrl_extra` as `Extra`, `mtrl_total` as `Purchased`, '' as `Sell` FROM `materials` WHERE `mtrl_acc_number` = (SELECT `acc_number` FROM `account` WHERE `acc_id` = 1) ";
                ...
                queryString += "UNION SELECT " +
                    "`mtrl_date` as `Date`, `mtrl_acc_number` as `Account`, `mtrl_item` as `Particulars`, `mtrl_rate` as `Rate`, `mtrl_qty` as `Qty`, `mtrl_extra` as `Extra`, '' as `Purchased`, `mtrl_total` as `Sell` FROM `materials` WHERE `mtrl_acc_number` != (SELECT `acc_number` FROM `account` WHERE `acc_id` = 1) ";
                if (!checkBoxSelectAll.Checked)
                {
                    queryString += "AND `mtrl_date` >= '" + dateTimePickerStartDate.Value.ToString("yyyy-MM-dd") + " 00:00:00' AND `mtrl_date` <= '" + dateTimePickerEndDate.Value.ToString("yyyy-MM-dd") + " 00:00:00' ";
                }
            ...
            else if (comboBoxAccountId.SelectedValue.Equals("PAYMENTS"))
            {
                queryString = "SELECT " +
            ...
            try
            {
                ...
                if (comboBoxAccountId.SelectedValue.Equals("MATERIALS"))
                {
